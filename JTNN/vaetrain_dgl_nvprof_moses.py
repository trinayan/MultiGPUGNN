...
dataset = JTNNDatasetMoses(data=opts.train, vocab=opts.vocab, training=True)
vocab = dataset.vocab
...
def train():
    ...
    for epoch in range(MAX_EPOCH):
        ...
        for it, batch in enumerate(tqdm.tqdm(dataloader)):
            model.zero_grad()
            profiling_iteration = profiling_iteration + 1
            try:
            ...
            print(profiling_iteration) 
            if(profiling_iteration == 20):
              break
